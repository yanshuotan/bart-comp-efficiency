...
# Import real dataset classification
from utils import ALLOWED_REAL_DATASETS
...
def modify_legend_linestyles(legend, exp_name: str, variations: list):
    """
    Modify legend line styles to use dotted lines for variations that have dashed plot lines.
    This makes the legend more visually distinct.
    """
    exp_key = exp_name.lower().replace('-', '').replace(' ', '')
    
    # Get experiments that use dashed lines
    dashed_experiments = {'initialization', 'dirichlet', 'marginalized', 'proposalmoves'}
    
    if exp_key in dashed_experiments:
        legend_lines = legend.get_lines()
        
        # Map each legend line to its corresponding variation
        for i, variation in enumerate(variations):
            if i < len(legend_lines):
                plot_linestyle = get_linestyle_for_variation(exp_name, variation)
                # If the plot line is dashed (--), make legend line dotted (:)
                if plot_linestyle == '--':
                    legend_lines[i].set_linestyle(':')
                    legend_lines[i].set_linewidth(3)  # Make dotted lines slightly thicker for visibility
...
def create_plots(df: pd.DataFrame, output_dir: Path, exp_name: str):
    ...
    # Note: Coverage metric will be chosen per-dataset based on real vs synthetic
    ...
    for dgp in df['dgp'].unique():
        ...
        for group_name, metrics_map in metric_groups.items():
            ...
            for metric_short_name, metric_full_name in metrics_map.items():
                ...
                # Clean up PMLB dataset names (remove ID prefix)
                import re
                clean_title = re.sub(r'^\d+\s*(Bng\s*)?', '', dgp_title)
                ...
                if dgp == 'low_lei_candes':
                    ...
                    clean_title = 'Breast Tumor'
                
                # Title: dataset name for coverage, empty space for others (for consistent height)
                if 'coverage' in metric_short_name:
                    fig.suptitle(clean_title, fontweight='bold')
                else:
                    fig.suptitle(' ', fontweight='bold')  # Empty title for consistent spacing
                ...
                if 'rmse' in metric_short_name and 'gr_rmse' not in metric_short_name:
                    legend = ax.legend(title="", frameon=True, ncol=1, loc='best')
                    # Modify legend line styles for better distinction
                    modify_legend_linestyles(legend, exp_name, variations)
                
                # Add reference lines
                if 'coverage' in metric_short_name:
                    ax.axhline(y=0.95, color='red', linestyle='--', linewidth=1.5, alpha=0.6)
                ...
                plot_path = output_dir / f"{exp_name}_{dgp_filename}_{metric_short_name}.pdf"
    ...
    for dgp in combined_df['dgp'].unique():
        ...
        for group_name, metrics_map in metric_groups.items():
            ...
            for metric_short_name, metric_full_name in metrics_map.items():
                ...
                if 'rmse' in metric_short_name and 'gr_rmse' not in metric_short_name:
                    legend = ax.legend(title="", frameon=True, ncol=1, loc='best')
                    # Modify legend line styles for better distinction - for combined plots, schedule variations are dashed
                    combined_variations = sorted(dgp_df['variation'].unique())
                    # For combined plots, manually set dotted legend lines for schedule variations
                    legend_lines = legend.get_lines()
                    for i, variation in enumerate(combined_variations):
                        if i < len(legend_lines):
                            # Schedule variations use dashed lines, so make legend dotted
                            if variation in ['Linear_3.0_to_1.0', 'linear']:  # Schedule variations
                                legend_lines[i].set_linestyle(':')
                                legend_lines[i].set_linewidth(3)
                ...
                # Save each metric as separate PDF (include exp_name in filename)
                plot_path = output_dir / f"Temperature_Schedule_{dgp_filename}_{metric_short_name}.pdf"
                plot_path.parent.mkdir(parents=True, exist_ok=True)
...
def main(cfg: DictConfig):
    ...
    # Fix path construction: if we're in experiments/ dir, artifact dir is relative to parent
    artifact_dir_str = exp_cfg.artifacts_dir
    if artifact_dir_str.startswith('experiments/'):
        base_artifact_dir = Path('..') / artifact_dir_str
    else:
        base_artifact_dir = Path(artifact_dir_str)
    
    LOGGER.info(f"Loading results for '{exp_name}' from: {base_artifact_dir}")
    
    df = load_results_to_dataframe(base_artifact_dir, experiment_name=exp_name, use_cache=False)
    ...
    if not df.empty:
        ...
        if exp_name in ["Temperature", "Schedule"] and cfg.get("create_combined_temp_schedule", False):
            ...
            # Fix path construction for combined plots
            temp_artifact_dir = Path('..') / temp_cfg.artifacts_dir if temp_cfg.artifacts_dir.startswith('experiments/') else Path(temp_cfg.artifacts_dir)
            sched_artifact_dir = Path('..') / sched_cfg.artifacts_dir if sched_cfg.artifacts_dir.startswith('experiments/') else Path(sched_cfg.artifacts_dir)
