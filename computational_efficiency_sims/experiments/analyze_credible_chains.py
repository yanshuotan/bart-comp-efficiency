...
def plot_gr_by_quantile(df, output_dir='plots_gr_quantiles'):
    ...
    for dgp in sorted(df['dgp'].unique()):
        ...
        # Custom labels for specific datasets
        dgp_label = dgp.replace('_', ' ').title()
        dgp_label = re.sub(r'^\d+\s*(Bng\s*)?', '', dgp_label)  # Remove ID prefix
        if dgp == 'low_lei_candes':
            dgp_label = 'Low Dimensional Smooth'
        elif dgp == 'piecewise_linear_kunzel':
            dgp_label = 'Piecewise Linear'
        elif dgp == '1199_BNG_echoMonths':
            dgp_label = 'Echo Months'
        elif dgp == '1201_BNG_breastTumor':
            dgp_label = 'Breast Tumor'
        
        ax.plot(quantiles, gr_means, marker='o', label=dgp_label, linewidth=2, markersize=8)
